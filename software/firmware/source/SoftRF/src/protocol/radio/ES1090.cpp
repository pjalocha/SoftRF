...
#if defined(ESP32) || defined(INCLUDE_LR2021)

static adsfo_t fo1090;  // EmptyFO1090;
static char buf1090[256];
static unsigned char *msg;
static unsigned char msgbuf[14];
...
static bool parse_df17(int index)
{
    yield();

    container_t *cip = &Container[index];

    mm.type = msg[4] >> 3;   // Extended squitter message type.
    mm.sub = msg[4] & 7;     // Extended squitter message subtype.

    if (mm.type >= 9 && mm.type <= 22 && mm.type != 19) {
        ...
        if (index < MAX_TRACKING_OBJECTS) {   // found
            ...
            // mark as transmitting ADS-B, even if rejected later (too far, etc)
            //if (fo1090.tx_type > cip->tx_type)
            //    cip->tx_type = fo1090.tx_type;
    ...
    if (index == MAX_TRACKING_OBJECTS)   // not found, and not a position message
        return false;

    if (mm.type == 19 && (mm.sub == 1 || mm.sub == 2)) {
...
#endif  // ESP32 || INCLUDE_LR2021

#if defined(ESP32)  // may have a GNS5892 attached
...
#endif  // ESP32 - may have a GNS5892 attached
...
bool es1090_decode(void *pkt, container_t *this_aircraft, ufo_t *fop) {

#if defined(INCLUDE_LR2021)

    msg = (unsigned char *) pkt;

    EmptyMsg(&mm);        // start with a clean slate of all zeros

    mm.frame = msg[0]>>3;    // Downlink Format

    // the LR2021 reception setup only detects DF17
    //if (mm.frame != 17) {
        // all other DFs
    //    return false;
    //}
    ...
    uint32_t addr = (msg[1] << 16) | (msg[2] << 8) | msg[3];   // ICAO ID
    if (addr == ThisAircraft.addr)        // somehow seeing ourselves
        return false;
    if (addr == settings->ignore_id)      // ID told in settings to ignore
        return false;
    ...
    (void) parse_df17(index);

    // The code above adds ADS-B traffic data into the traffic table directly.
    // (That is because different data items arrive in separate ADS-B packets.)
    // Thus after returning to TrafficHelper ParseData() there is nothing left to do.
    // Therefore always return false here, as if the packet was rejected.
...
size_t es1090_encode(void *pkt, container_t *this_aircraft) {
